...
void stackMenu() {
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 7: {
                int sum = stack.Reduce(0, [](const int& a, const int& b) -> int { return a + b; });
...
void dequeMenu() {
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 10: {
                int sum = deq.Reduce(0, [](const int& a, const int& b) -> int { return a + b; });
                std::cout << "Sum: " << sum << std::endl;
                break;
            }
            case 11: {
            ...
            case 12: {
                ...
                other.Sort();
                Deque<int> merged = Deque<int>::MergeDeques(deq, other);
                std::cout << "Merged: ";
                for (int i = 0; i < merged.GetLength(); ++i)
                    std::cout << merged.GetSequence()->Get(i) << " ";
                std::cout << std::endl;
                break;
            }
            case 13: {
...
void run_interface() {
    ...
    try {
        if (type == 5) {
            ...
            std::cout << "Queue created (" << queue->GetLength() << " items)\n";
        ...
        while (true) {
            ...
            try {
                if (qchoice == 1) {
                    ...
                    std::cout << "Subsequence (size " << sub->GetLength() << "): ";
                    int subSize = sub->GetLength();
                    ...
                    std::cout << "Part 1 (Evens, size " << parts.first->GetLength() << ")\n";
                    std::cout << "Part 2 (Odds, size " << parts.second->GetLength() << ")\n";
                    delete parts.first;
                    delete parts.second;
                } else if (qchoice == 12) {
