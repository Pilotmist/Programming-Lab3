...
private:
    Sequence<T>* elements;
public:
    Vector() : elements(new MutableArraySequence<T>()) {}
    explicit Vector(int size) : elements(new MutableArraySequence<T>()) {
        for (int i = 0; i < size; ++i) elements->Append(T());
    }
    explicit Vector(Sequence<T>* sequence) : elements(sequence) {}

    Vector(const Vector& other) {
        this->elements = new MutableArraySequence<T>();
        int size = other.Size();
        for (int i = 0; i < size; ++i) this->elements->Append(other.GetAt(i));
    }
    ...
    Vector& operator=(const Vector& other) {
        if (this != &other) {
            ...
            int size = other.Size();
            for (int i = 0; i < size; ++i) newElements->Append(other.GetAt(i));
            delete elements;
    ...
    int Size() const { return elements->GetLength(); }
    T GetAt(int index) const { return elements->Get(index); }
    ...
    Vector<T> SumWith(const Vector<T>& other) const {
        int size = Size();
        if (size != other.Size()) throw std::domain_error("Vector sizes must match for addition");
    ...
    T DotProduct(const Vector<T>& other) const {
        int size = Size();
        if (size != other.Size()) throw std::domain_error("Vector sizes must match for dot product");
        T result = T();
        for (int i = 0; i < size; ++i) {
